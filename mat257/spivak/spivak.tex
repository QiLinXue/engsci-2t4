...
\end{equation}
so the weird things with rearrangements do not occur.
\subsection{Theorem}
We wish to show that 
\begin{theorem}
    If $A$ and $f$ are as before, then 
    \begin{enumerate}
        \item If $U,U'$ and $\Phi,\Phi'$ are open covers and partitions of unity as before, then 
        \begin{equation}
            \int^{(U',\Phi')}_{A} f = \int^{(U,\Phi)}_{A} f.
        \end{equation}
        In that case,
        \begin{equation}
            \int^{NT}\limits_{A} f := \int^{(U',\Phi')}_{A} f = \int^{(U,\Phi)}_{A} f.
        \end{equation}
    \end{enumerate}
\end{theorem}
\end{document}